"""Visualize v12_reg predictions across LUDB / ISP / QTDB test sets.

Layout: signal + GT + v12_reg (cls + reg-refined boundaries) + NK + WT.
Same plot structure as viz_v9_q1c_pu_merge but using FrameClassifierViTReg
+ predict_frames_with_reg + apply_reg_to_boundaries.
"""
...
from scripts.viz_test_all_datasets import (
    ...
    parse_wfdb_ann, plot_panel, FRAME_MS, FS, WINDOW_SAMPLES,
)

CKPT = REPO / "data" / "checkpoints" / "stage2_v12_reg.pt"
OUT_BASE = REPO / "out" / "viz_v12_reg"


def predict_one(model, sig, lead_idx, device):
    """Run v12_reg inference and apply reg head's offset to each boundary.

    Returns (boundaries dict with refined sample indices, color bands from
    the post-processed frame argmax).
    """
