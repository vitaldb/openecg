"""v8 d_model sweep — isolate d_model effect with n_layers=8 fixed.

Tests how small d_model can go before performance collapses.

All variants share:
  - ViT, patch=5, learnable pos, no_lead_emb, no conv_stem
  - n_layers=8, ff=4*d_model (standard expansion ratio)
  - LUDB train (edge-masked) + ISP train + QTDB q1c sliding (same as v7_pure_q1c)

Variants:
  v8_dim_5   : d=5,   heads=1, head_dim=5,  ff=32
  v8_dim_16  : d=16,  heads=4, head_dim=4,  ff=64
  v8_dim_32  : d=32,  heads=4, head_dim=8,  ff=128
  v8_dim_64  : d=64,  heads=4, head_dim=16, ff=256
  v8_dim_128 : reuse v7_pure_q1c.pt for eval (already trained, l size)
"""
...
# n_layers=8 fixed; ff=4*d (standard ratio); n_heads scaled to keep head_dim>=8
# (d=5 and d=16 were tested and dropped — d=5 too small for P detection,
#  d=16 had non-standard head_dim=4 with weak P=0.30 even at epoch 28)
DIM_CONFIGS = {
    "v8_dim_32":  dict(d_model=32,  n_heads=4, ff=128, n_layers=8),
    "v8_dim_64":  dict(d_model=64,  n_heads=4, ff=256, n_layers=8),
}
...
def build_train_dataset_q1c():
    print("Building train dataset (q1c QTDB)...", flush=True)
...
def main():
    ...
    ludb_val = LUDBFrameDataset(ludb.load_split()["val"],
                                  mask_unlabeled_edges=True,
                                  edge_margin_ms=EDGE_MARGIN_MS)
    train_ds = build_train_dataset_q1c()
    full = {}

    for name, dim_kwargs in DIM_CONFIGS.items():
        cfg = {**BASE_KWARGS, **dim_kwargs}
    ...
    # Reuse v7_pure_q1c (= v8_dim_128) for eval comparison
    print("\n>>> v8_dim_128 (reuse v7_pure_q1c.pt) <<<", flush=True)
    cfg_l = {**BASE_KWARGS, "d_model": 128, "n_heads": 4, "ff": 256, "n_layers": 8}
    model_l = FrameClassifierViT(**cfg_l).to(device)
    load_checkpoint(CKPT_DIR / "stage2_v7_pure_q1c.pt", model_l)
    model_l = model_l.train(False)
    n_params_l = sum(p.numel() for p in model_l.parameters())
    res_l = eval_all(model_l, device)
    full["v8_dim_128"] = {"params": n_params_l, "train_seconds": 0.0,
                           "config": cfg_l, **res_l}
    for k, v in full["v8_dim_128"].items():
        if isinstance(v, float):
            print(f"  {k}: {v:.3f}")

    # Summary
    print("\n" + "=" * 100, flush=True)
    print(f"{'v8 D_MODEL SWEEP SUMMARY (n_layers=8 fixed, ff=4d, q1c train)':^100}", flush=True)
    print("=" * 100, flush=True)
    print(f"  {'model':14s} {'params':>10s} | {'LUDB(edge)':>10s} {'ISP':>10s} {'QTDB pu0':>10s}", flush=True)
    ...
    out_path = OUT_DIR / f"train_v8_dim_sweep_{ts}.json"
