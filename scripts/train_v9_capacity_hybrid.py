"""v9 — capacity scan + hybrid q1c label fix.

Building on v7_pure_q1c (best so far: LUDB 0.923 / ISP 0.941):

  v9_d192       : d=192, L=8 (wider, ~2.4M params) — test if d saturates
  v9_d128_L12   : d=128, L=12 (deeper, ~1.6M params) — depth vs width
  v9_d256_L8    : d=256, L=8 (much wider, ~4.7M params) — capacity ceiling
  v9_hybrid     : d=128, L=8, q1c with ignore_missing_waves=True
                  (mark estimated T region as IGNORE when q1c lacks t_on)

All variants share: ViT, patch=5, learnable pos, no_lead_emb, no conv_stem,
LUDB train (edge-masked) + ISP train + QTDB q1c sliding.
...
def build_train_dataset(ignore_missing_waves=False, q1c_pu_merge=False):
    tag = ""
    if ignore_missing_waves: tag = " (IGNORE missing)"
    elif q1c_pu_merge: tag = " (q1c+pu0 merge)"
    print(f"Building train dataset{tag}...", flush=True)
    ludb_train = LUDBFrameDataset(ludb.load_split()["train"],
                                    mask_unlabeled_edges=True,
                                    edge_margin_ms=EDGE_MARGIN_MS)
    print(f"  LUDB train: {len(ludb_train)}", flush=True)
    isp_train = CombinedFrameDataset(["isp_train"])
    print(f"  ISP train: {len(isp_train)}", flush=True)
    qtdb_q1c = QTDBSlidingDataset(scale_factors=(1.0,), windows_per_record=20,
                                    seed=SEED,
                                    ignore_missing_waves=ignore_missing_waves,
                                    q1c_pu_merge=q1c_pu_merge)
    print(f"  QTDB q1c sliding: {len(qtdb_q1c)}", flush=True)
    return _ConcatWithCounts([ludb_train, isp_train, qtdb_q1c])


# Eval functions (same as v7/v8) -----------------------------------------------
...
def main():
    ...
    # Reference: v7_pure_q1c (= v8_dim_128 = v9 base)
    full = {
        "v7_pure_q1c_ref": {
            "params": 1126660, "ludb_edge_filtered": 0.923,
            "isp_test": 0.941, "qtdb_pu0_random": 0.747,
        },
    }

    # Standard q1c train dataset
    train_ds_std = build_train_dataset(ignore_missing_waves=False)

    # v9_d192: wider
    cfg = {**BASE_KWARGS, "d_model": 192, "n_layers": 8, "ff": 768}
    model, elapsed, n_params = train_one("v9_d192", train_ds_std, ludb_val,
                                          cfg, device, CKPT_DIR / "stage2_v9_d192.pt")
    res = eval_all(model, device)
    full["v9_d192"] = {"params": n_params, "train_seconds": elapsed,
                        "config": cfg, **res}
    ...
    # v9_d128_L12: deeper
    cfg = {**BASE_KWARGS, "d_model": 128, "n_layers": 12, "ff": 256}
    model, elapsed, n_params = train_one("v9_d128_L12", train_ds_std, ludb_val,
                                          cfg, device, CKPT_DIR / "stage2_v9_d128_L12.pt")
    res = eval_all(model, device)
    full["v9_d128_L12"] = {"params": n_params, "train_seconds": elapsed,
                            "config": cfg, **res}
    for k, v in res.items(): print(f"  {k}: {v:.3f}")
    del model
    if device == "cuda": torch.cuda.empty_cache()

    # v9_d256_L8: much wider
    cfg = {**BASE_KWARGS, "d_model": 256, "n_layers": 8, "ff": 1024}
    model, elapsed, n_params = train_one("v9_d256_L8", train_ds_std, ludb_val,
                                          cfg, device, CKPT_DIR / "stage2_v9_d256_L8.pt")
    res = eval_all(model, device)
    full["v9_d256_L8"] = {"params": n_params, "train_seconds": elapsed,
                           "config": cfg, **res}
    for k, v in res.items(): print(f"  {k}: {v:.3f}")
    del model, train_ds_std
    if device == "cuda": torch.cuda.empty_cache()

    # v9_hybrid: q1c + ignore_missing_waves (T region IGNORE when t_on missing)
    train_ds_hybrid = build_train_dataset(ignore_missing_waves=True)
    cfg = {**BASE_KWARGS, "d_model": 128, "n_layers": 8, "ff": 256}
    model, elapsed, n_params = train_one("v9_hybrid", train_ds_hybrid, ludb_val,
                                          cfg, device, CKPT_DIR / "stage2_v9_hybrid.pt")
    res = eval_all(model, device)
    full["v9_hybrid"] = {"params": n_params, "train_seconds": elapsed,
                          "config": cfg, **res}
    for k, v in res.items(): print(f"  {k}: {v:.3f}")

    # Summary
    print("\n" + "=" * 100, flush=True)
    print(f"{'v9 CAPACITY + HYBRID SUMMARY: avg Martinez F1 by domain':^100}", flush=True)
    print("=" * 100, flush=True)
    print(f"  {'model':18s} {'params':>10s} | {'LUDB(edge)':>10s} {'ISP':>10s} {'QTDB pu0':>10s}", flush=True)
    for name, r in full.items():
        print(f"  {name:18s} {r['params']:>10,d} | "
              f"{r['ludb_edge_filtered']:>10.3f} {r['isp_test']:>10.3f} "
              f"{r['qtdb_pu0_random']:>10.3f}", flush=True)

    ts = time.strftime("%Y%m%d_%H%M%S")
    out_path = OUT_DIR / f"train_v9_capacity_hybrid_{ts}.json"
    def _safe(v):
