"""Retrain v12_reg with the AV-block synthetic dataset added to the mix.

The previous v12_reg was trained on LUDB + ISP + QTDB only — three sources
that all couple P to a following QRS within ~200 ms. This retrain adds
synthetic Mobitz I / II / complete AV-block windows so the model sees the
"orphan P-wave" pattern at training time.

Mix:
  - LUDB train (mask_unlabeled_edges=True)        — boundary GT, real
  - ISP train                                      — boundary GT, real
  - QTDBSlidingDataset (q1c+pu0 merge)             — boundary GT, real
  - SyntheticAVBDataset (~25% of mix by default)   — boundary GT, synthetic

Output:
  data/checkpoints/stage2_v12_reg_with_synth.pt    (new canonical for the
    AV-block fix; previous stage2_v12_reg.pt left in place to A/B against.)
  out/retrain_v12_reg_with_synth_<ts>.json
"""
import sys
import time
...
OUT_DIR = REPO / "out"
LAMBDA = 0.05
N_SYNTH_WINDOWS = 2000   # synthetic windows per epoch (~13-17% of full mix)
...
def main():
    ...
    print(f"Synthetic AVB dataset: {len(synth_ds)} windows", flush=True)
    ...
    ckpt_path = CKPT_DIR / "stage2_v12_reg_with_synth.pt"
    print(f"\n=== TRAIN v12_reg + synth (lambda={LAMBDA}, "
    ...
    out_path = OUT_DIR / f"retrain_v12_reg_with_synth_{ts}.json"
    import json
