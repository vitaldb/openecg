"""v7_pure_q1c size sweep — find smallest model that matches l-size performance.

v7_pure_q1c (l) achieved LUDB 0.923 / ISP 0.941. Test if smaller variants
(s, m) can reach similar performance with fewer params.

All variants share:
  - ViT arch, patch=5, learnable pos, no_lead_emb, no conv_stem
  - LUDB train (edge-masked) + ISP train + QTDB q1c sliding (33 records)

Sizes:
  s : d_model=64,  n_layers=4, ff=128
  m : d_model=96,  n_layers=6, ff=192
  l : d_model=128, n_layers=8, ff=256  (already trained, eval-only)
"""
...
SIZES = {
    "s": dict(d_model=64,  n_layers=4, ff=128),
    "m": dict(d_model=96,  n_layers=6, ff=192),
    "l": dict(d_model=128, n_layers=8, ff=256),
}
...
def main():
    ...
    # Reuse trained l checkpoint if available
    l_ckpt = CKPT_DIR / "stage2_v7_pure_q1c.pt"
    for size_label, size_kwargs in SIZES.items():
        name = f"v7_q1c_{size_label}"
        cfg = {**BASE_KWARGS, **size_kwargs}
        ckpt_path = (l_ckpt if size_label == "l" else
                      CKPT_DIR / f"stage2_{name}.pt")
        if size_label == "l" and ckpt_path.exists():
            print(f"\n>>> {name} (reuse {l_ckpt.name}) <<<", flush=True)
            model = FrameClassifierViT(**cfg).to(device)
            load_checkpoint(ckpt_path, model)
    ...
    # Summary
    print("\n" + "=" * 100, flush=True)
    print(f"{'v7 q1c SIZE SWEEP SUMMARY: avg Martinez F1 by domain':^100}", flush=True)
    ...
    out_path = OUT_DIR / f"train_v7_q1c_size_sweep_{ts}.json"
