"""Visualize v7_pure_q1c predictions across LUDB / ISP / QTDB test sets.
Same layout as viz_test_all_datasets.py but loads ViT model on CPU
(so it doesn't compete with concurrent training on GPU).
"""
...
CKPT = REPO / "data" / "checkpoints" / "stage2_v7_pure_q1c.pt"
OUT_BASE = REPO / "out" / "viz_v7"
    ...
    for rid in rec_ids:
        for lead in leads:
            ...
            if len(sig) < WINDOW_SAMPLES:
                continue
            bds, bands = v4_predict(model, sig, lead_idx, device)
            try:
...
def viz_qtdb(model, device, n_records=6):
    ...
    for rid in rids:
        ...
        bds, bands = v4_predict(model, sig_for_v4, 1, device)
...
def main():
    ...
    model = FrameClassifierViT(**V7_KWARGS)
